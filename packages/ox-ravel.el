...
(defcustom org-ravel-engines '(("R"))
  ...
  Buffer local values are allowed."
   ...
   :type '(set :greedy t
	       ...
	       (const :tag "   plantuml" ("plantuml" . "engine='plantuml' comment=\"'''\""))
...
(defun org-ravel-rewrap (retval &optional inline engine-cdr)
  "(Re)Set `:wrap', `:results', `:exports', amd `:engine'
     ...
     differ. ENGINE-CDR gives the engine string, if any.
  	...
  	The original header args `:exports', `:wrap', `:file', `:file-ext', 
  	`:results' and `:result-params' get suffixed with `-arg'. Block/snippet style
  	functions can find them in `R-HEADERS-ATTR'. "
    (let ((n2r (nth 2 retval)))
      (cl-loop
       for carname in
       '(:exports :results :wrap :file :file-ext :result-params) do
...
(defun ox-ravel--args-alist (argstring)
  ...
  (with-temp-buffer
	...
	(ess-jump-to-next-arg))
	  ...
	  (and (not blst) (< 0 (length argstring))
	       (warn "apparent parse error in %s" argstring))
	  (reverse blst))))
