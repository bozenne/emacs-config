...
(defun brice-ess-packageSource-r ()
  ...
  (interactive)
  (if (or (eq major-mode 'ess-r-mode)  (eq major-mode 'c++-mode))
	  ;; (setq mylist (list "butils.base" "butils" "BuyseTest" "lavaSearch2" "riskRegression")
	  (let ((mylist (list "butils" "butils.base" "BuyseTest" "DelayedGSD" "lavaSearch2" "lmbreak" "LMMstar" "riskRegression")))
...
(defun brice-ess-dim-object ()
  ...
  (if (or (eq major-mode 'ess-r-mode)  (eq major-mode 'inferior-ess-mode))
      (if (region-active-p)
	  (let ((x (buffer-substring (region-beginning) (region-end))))
	    (ess-eval-linewise (concat "butils.base::DIM(" x ")")))
   	   (let ((x (read-string "Enter a the name of the object:")))
	     (ess-eval-linewise (concat "butils.base::DIM(" x ")")))
	   )
      (message "works only with .R files or R terminals")
...
(defun brice-ess-names-object ()
  ...
  (if (or (eq major-mode 'ess-r-mode)  (eq major-mode 'inferior-ess-mode))
      (if (region-active-p)
	  (let ((x (buffer-substring (region-beginning) (region-end))))
	    (ess-eval-linewise (concat "butils.base::NAMES(" x ")")))
   	   (let ((x (read-string "Enter a the name of the object:")))
	     (ess-eval-linewise (concat "butils.base::NAMES(" x ")")))
	   )
      (message "works only with .R files or R terminals")
...
(defun brice-ess-script-object ()
  ...
  (if (or (eq major-mode 'ess-r-mode)  (eq major-mode 'inferior-ess-mode))
      (if (region-active-p)
	  (let ((x (buffer-substring (region-beginning) (region-end))))
	    (ess-eval-linewise (concat "butils.base::object2script(" x ", digit = 8)")))
   	   (let ((x (read-string "Enter a the name of the object:")))
	     (ess-eval-linewise (concat "butils.base::object2script(" x ", digit = 8)")))
	   )
      (message "works only with .R files or R terminals")
...
(defun brice-ess-unique-object ()
  "Display the dimension of an R object"
  (interactive)
  (if (or (eq major-mode 'ess-r-mode)  (eq major-mode 'inferior-ess-mode))
      (if (region-active-p)
	  (let ((x (buffer-substring (region-beginning) (region-end))))
	    (ess-eval-linewise (concat "butils.base::UNIQUE(" x ")")))
   	   (let ((x (read-string "Enter a the name of the object:")))
	     (ess-eval-linewise (concat "butils.base::UNIQUE(" x ")")))
	   )
      (message "works only with .R files or R terminals")
